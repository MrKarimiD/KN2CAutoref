...
bool MapSearchNode::GetSuccessors(AStarSearch<MapSearchNode> *astarsearch, MapSearchNode *parent_node)
{
    ...
    //----------
    Position rpos = wm->autoReplacer.pos;
...
QList<Circle2D> MapSearchNode::getObsCircle()
{
    ...
    for(int i=0; i<PLAYERS_MAX_NUM; i++)
    {
        ...
        if(!wm->blueRobot[i].isValid) continue;
        //if((wm->blueRobot[i].pos.loc-wm->blueRobot[selfRobot].pos.loc).length() < 500
         //       && (wm->blueRobot[i].vel.loc-wm->blueRobot[selfRobot].vel.loc).length() < 1) continue;
        Circle2D c(wm->blueRobot[i].pos.loc + wm->blueRobot[i].vel.loc * AI_TIMER, r_rad);
        result.append(c);
    }

    for(int i=0; i<PLAYERS_MAX_NUM; i++)
    {
        if(!wm->yellowRobot[i].isValid) continue;
        Circle2D c(wm->yellowRobot[i].pos.loc+ wm->yellowRobot[i].vel.loc * AI_TIMER, r_rad);
        result.append(c);
    }
