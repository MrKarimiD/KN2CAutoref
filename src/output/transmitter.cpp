    ...
    while (_serialport.bytesAvailable())
    {
        ...
        else if (read_state >= PRE_2)
        {
            ...
            if(read_state == 120)
            {
                ...
                for(int i=0; i<12; i++)
                {
                    _wm->autoReplacer.rd.M0 = readbuffer[1 + i*10]*256 + readbuffer[0 + i*10];
                    _wm->autoReplacer.rd.M1 = readbuffer[3 + i*10]*256 + readbuffer[2 + i*10];
                    _wm->autoReplacer.rd.M2 = readbuffer[5 + i*10]*256 + readbuffer[4 + i*10];
                    _wm->autoReplacer.rd.M3 = readbuffer[7 + i*10]*256 + readbuffer[6 + i*10];
                    _wm->autoReplacer.rd.KCK = readbuffer[8 + i*10];
                    _wm->autoReplacer.rd.CHP = readbuffer[9 + i*10];
                }
                updateVel2();
            }
        }
...
void Transmitter::updateVel2()
{
    for(int i=0; i<12; i++)
    {
        ...
        speed[0] = rotate[0][0] * _wm->autoReplacer.rd.M0 + rotate[0][1] * _wm->autoReplacer.rd.M1 + rotate[0][2] * _wm->autoReplacer.rd.M2 + rotate[0][3] * _wm->autoReplacer.rd.M3;
        speed[1] = rotate[1][0] * _wm->autoReplacer.rd.M0 + rotate[1][1] * _wm->autoReplacer.rd.M1 + rotate[1][2] * _wm->autoReplacer.rd.M2 + rotate[1][3] * _wm->autoReplacer.rd.M3;
        speed[2] = rotate[2][0] * _wm->autoReplacer.rd.M0 + rotate[2][1] * _wm->autoReplacer.rd.M1 + rotate[2][2] * _wm->autoReplacer.rd.M2 + rotate[2][3] * _wm->autoReplacer.rd.M3;

        speed[0] /= 1375.14;
        speed[1] /= 1375.14;
        speed[2] /= 1375.14;

        _wm->autoReplacer.vel2.loc.x = cos(_wm->autoReplacer.pos.dir) * speed[0] - sin(_wm->autoReplacer.pos.dir) * speed[1];
        _wm->autoReplacer.vel2.loc.y = sin(_wm->autoReplacer.pos.dir) * speed[0] + cos(_wm->autoReplacer.pos.dir) * speed[1];
        _wm->autoReplacer.vel2.dir = speed[2];
    }
