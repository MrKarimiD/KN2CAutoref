...
void RenderArea::paintEvent(QPaintEvent *)
{
    ...
    for(int i=0;i<12;i++)
        if(_sc->wm->yellowRobot[i].isValid)
        {
            painter.drawChord(_sc->wm->yellowRobot[i].pos.loc.x/WORLD_SCALE-ROBOT_R,-_sc->wm->yellowRobot[i].pos.loc.y/WORLD_SCALE-ROBOT_R,ROBOT_D,ROBOT_D,ANGLE_TO_CHORD(_sc->wm->yellowRobot[i].pos.dir));
            painter.drawText(_sc->wm->yellowRobot[i].pos.loc.x/WORLD_SCALE-ROBOT_R,-_sc->wm->yellowRobot[i].pos.loc.y/WORLD_SCALE-ROBOT_R,ROBOT_D,ROBOT_D,Qt::AlignCenter,QString::number(i));
        }

    painter.setBrush(*brush_brobot);
    for(int i=0;i<12;i++)
        if(_sc->wm->blueRobot[i].isValid)
        {
            painter.drawChord(_sc->wm->blueRobot[i].pos.loc.x/WORLD_SCALE-ROBOT_R,-_sc->wm->blueRobot[i].pos.loc.y/WORLD_SCALE-ROBOT_R,ROBOT_D,ROBOT_D,ANGLE_TO_CHORD(_sc->wm->blueRobot[i].pos.dir));
            painter.drawText(_sc->wm->blueRobot[i].pos.loc.x/WORLD_SCALE-ROBOT_R,-_sc->wm->blueRobot[i].pos.loc.y/WORLD_SCALE-ROBOT_R,ROBOT_D,ROBOT_D,Qt::AlignCenter,QString::number(i));
        }
